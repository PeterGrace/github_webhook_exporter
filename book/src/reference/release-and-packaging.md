...
For example, after all four version fields are `0.1.8`, consume the published image and chart:

```bash
docker pull ghcr.io/petergrace/github-webhook-exporter:0.1.8
helm pull oci://ghcr.io/petergrace/charts/github-webhook-exporter --version 0.1.8
helm install github-webhook-exporter oci://ghcr.io/petergrace/charts/github-webhook-exporter --version 0.1.8
```
...
`dist/github-webhook-exporter-0.1.8.tgz`. `just helm-kind-acceptance` confirms API acceptance for
...
```bash
helm show chart dist/github-webhook-exporter-0.1.8.tgz
helm show values dist/github-webhook-exporter-0.1.8.tgz
helm template archive dist/github-webhook-exporter-0.1.8.tgz --kube-version 1.35.0 >/dev/null
```
